frontmatter='''An independent script to evaluate over a range
'''
print(frontmatter)
...
if os.path.exists(filename_checkpoint):
        ...
        with torch.no_grad():
            ...
            if evaluation_only:
                if True:
                    print('start test X_test.shape',X_test.shape)
                    def _eval(_X,_y):
                        _y_pred = model(_X)
                        _loss = get_loss(_y,_y_pred, debug=False).detach().cpu().item() 
                        return _loss
                    y_pred = model(X_test)
                    _loss = get_loss(y_test,y_pred, debug=False).detach().cpu().item()  
                    print('overall loss',_loss)

                    division_values = [-0.1*_ for _ in range(13,38)]
                    division_values.reverse()
                    def divide_and_eval(X_test0,y_test0,index,recursive = True):
                        """sort based on the index'th column, divided into small ranges and return loss in each range"""
                        _ ,indices = X_test0[:,index].sort()  # sort according to column index
                        X_eval = X_test0[indices]
                        y_eval = y_test0[indices]
                        #np.linspace(-3.8,-1.2,0.1)
                        
                        division_indices = [0]
                        for value in division_values:
                            ind = (X_eval[indices][:,index]<value).sum().item()
                            division_indices.append(ind)
                        division_indices.append(-1)
                        _loss_list=[]
                        _loss_array=[]
                        for i in range(len(division_indices)-1):
                            ind1,ind2 = division_indices[i],division_indices[i+1]
                            _X = X_eval[ind1:ind2]
                            _y = y_eval[ind1:ind2]
                            #print(f'_X at division {i}')
                            #print(_X)
                            #input()
                            _loss = _eval(_X,_y)
                            _loss_list.append(_loss)
                            if recursive:
                                _ = divide_and_eval(_X,_y,index = index2,recursive=False)
                                _loss_array.append(_)
